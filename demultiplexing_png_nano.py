...
def demultiplex():
    ...
    for sfn in site_file_name_lists:
        ...
        for index in index_list:
            r1_demultiplexed_file = '{}/{}_{}_r1.fastq.gz'.format(sfn_dir, sfn, index)
            r2_demultiplexed_file = '{}/{}_{}_r2.fastq.gz'.format(sfn_dir, sfn, index)
        ...
        with open(tab_delim_path, 'w') as f:
            for line in tab_delim_file:
                f.write('{}\n'.format(line))
        r1_reads_no_match_path = '{}/no_bc_match_r1.fastq.gz'.format(sfn_dir)
        r2_reads_no_match_path = '{}/no_bc_match_r2.fastq.gz'.format(sfn_dir)

        # now run sabre
        subprocess.run(['sabre', 'pe', '-f', r1_sfn_file, '-r', r2_sfn_file, '-b', tab_delim_path,
                        '-u', r1_reads_no_match_path, '-w', r2_reads_no_match_path, '-m', '0'])
